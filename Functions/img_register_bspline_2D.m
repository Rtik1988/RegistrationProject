...
while (current_factor > STOP_OPT_FACTOR)
    ...
    for i=1:LINE_SEARCH_STEPS
        ...
        possible_O(i+1,1) = img_distance(It, Itemp, metric, 0);
    end
    ...
    if Options.Verbose      
        ...
        subplot(1,4,1);
        imagesc(round(It)./255);
        title('target');
        subplot(1,4,2);
        imagesc(round(Itemp)./255);
        title('intermidiate');
        subplot(1,4,3);
        imagesc(abs(round(Is-It))./255,[0,0.5]);
        title('Abs. diff before');
        subplot(1,4,4);
        imagesc(abs(round(Itemp-It))./255,[0,0.5]);
        title('Abs. diff');
        drawnow;
